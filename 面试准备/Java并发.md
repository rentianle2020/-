...
> 方法区放置于Heap中的Permgen，可能导致内存泄露（垃圾没有被合理回收），报错OutOfMemoryException
>
...
轻量级锁：使用CAS将对象头中的线程指针指向自己；自旋次数超出阈值，则升级锁
...
isTerminated()：调用shutdown方法，并完成所有剩余任务后返回true
...
- CPU密集型任务：N + 1，多出来的一个线程，以防某个线程偶发的缺页终端去IO，来充分利用CPU
...
- **共享变量**没有锁定，将线程设置为当前线程，并锁定资源
- 共享已经被锁定，将线程加入**队列**并阻塞，等待唤醒

acquire(int args)：通过先加入队列，试图cas失败，设置predecessor为Signal，再试图cas，最后阻塞线程。

release(int args)：head（也就是prev）被查询为Signal，先将successor出队，再唤醒successor线程，唤醒后检查是否被Interrupt()，interrupt本身就是一个标志位，如果我们没有拿他做任何事情（比如检查并抛异常），它本身没有意义。

> Thread.currentThread().interrupt()设置标志位，Thread.currentThread().isInterrupted()检查标志位，Thread.interrupted()返回当前标志位并将其清除。
>
> Unsafe的park()函数，就会在Thread()被interrupt时返回，使得Thread恢复调度。由于是native方法，没有阅读过代码，但是注释是这么说的。

问题：如果在阻塞前被唤醒怎么办？Unsafe.unpark()注解中说明，如果没有被block，则下一个park失效。
...
可以通过newCondition()去awit()和signal()指定condition中的队列。
